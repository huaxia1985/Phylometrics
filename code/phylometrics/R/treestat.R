...
#' @examples
#' Here we conduct significane test on a metric defined by 'tars' function for a simulated phylogeny. The null hypothesis is that the trait is randomly distributed across tips. We don't know if the trait will have larger or smaller metric value than expected, so we apply a "two.sided" test. 1000 traits are simulated to generate the null distribution of the metric.
...
#' treestat(phy, func=tars, traitevol="random", a=1000, alternative="two.sided", simplify=T)

#' Becasue trait simulation takes time, users can conduct significance test on the same simulated traits for multiple metrics with different options of alternative hypotheses
#' treestat(phy, func=list(tars,noto), traitevol="random", a=1000, alternative=("two.sided","greater"), simplify=T)
